...
import { getStoredCombustible, saveStoredCombustible, filterByRange } from '../utils/storage';
import { 
  Plus, 
  Edit2, 
  Trash2, 
...
export default function GastosCombustible({ 
  ...
  const handleDeleteItem = (id: string) => {
    if (window.confirm('¿Está seguro de que desea eliminar este registro de carga de combustible?')) {
      ...
      saveStoredCombustible(updated, userId);
  ...
  const handleSaveItem = (e: React.FormEvent) => {
    ...
    let updatedCombustibles: GastoCombustible[] = [];

    if (editingItem) {
      // Edit
      updatedCombustibles = combustibles.map(c => c.id === editingItem.id
        ? { ...c, fecha, tipo, importe: numImporte, nota }
        : c
      );
    } else {
      // Add
      const newItem: GastoCombustible = {
        id: `c-${Date.now()}`,
        userId,
        fecha,
        tipo,
      ...
      updatedCombustibles = [newItem, ...combustibles];
    }

    setCombustibles(updatedCombustibles);
    saveStoredCombustible(updatedCombustibles, userId);
