...
import { getStoredViajes, saveStoredViajes, filterByRange } from '../utils/storage';
import { 
  Plus, 
  Edit2, 
  Trash2, 
...
export default function Viajes({ userId, globalFilterRange, setGlobalFilterRange }: ViajesProps) {
  ...
  const REFERENCE_DATE = '2026-07-08'; // System reference date
  ...
  const handleDeleteTrip = (id: string) => {
    if (window.confirm('¿Está seguro de que desea eliminar este viaje?')) {
      ...
      setViajes(updated);
      saveStoredViajes(updated, userId);
  ...
  const handleSaveTrip = (e: React.FormEvent) => {
    ...
    let updatedViajes: Viaje[] = [];

    if (editingTrip) {
      // Edit
      updatedViajes = viajes.map(v => v.id === editingTrip.id 
        ? { ...v, fecha, formaPago, monto: numMonto } 
        : v
      );
    } else {
      // Add
      ...
      updatedViajes = [newTrip, ...viajes];
    }

    setViajes(updatedViajes);
    saveStoredViajes(updatedViajes, userId);
