...
export default function Login({ onLoginSuccess, onNavigateToRegister }: LoginProps) {
  ...
  const handleLogin = (e: React.FormEvent) => {
    ...
    if (!matchedUser) {
      setErrorMsg('Usuario o correo electrónico no registrado.');
      return;
    }

    // Support both '¡TaxiControl' and 'TaxiControl' for the default user Mario
    let isCorrect = matchedUser.passwordHash === hash;
    if (!isCorrect && matchedUser.username === 'Mario') {
      const altPassword = password === 'Taxicontrol!' ? 'Taxicontrol!' : (password === 'Taxicontrol!' ? 'Taxicontrol!' : null);
      if (altPassword) {
        const altHash = encryptPassword(altPassword);
        if (matchedUser.passwordHash === altHash) {
          isCorrect = true;
          // Sync/Update their passwordHash to the password they entered to avoid future mismatches
          matchedUser.passwordHash = hash;
          const updatedUsersList = users.map(u => u.id === matchedUser.id ? matchedUser : u);
          localStorage.setItem('taxi_users', JSON.stringify(updatedUsersList));
        }
      }
    }
              ...
              <div>
                ...
                <div className="relative">
                  ...
                  <input
                    ...
                    placeholder="Ej: Mario o mario@taxicontrol.com"
                  />
                </div>
              </div>
