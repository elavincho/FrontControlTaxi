...
export default function MantenimientoComponent({ 
  ...
  const handleDeleteItem = (id: string) => {
    if (window.confirm('¿Está seguro de que desea eliminar esta tarea de mantenimiento?')) {
      ...
      saveStoredMantenimiento(updated, user.id);
  ...
  const handleSaveItem = (e: React.FormEvent) => {
    ...
    let updatedMants: Mantenimiento[] = [];
    const itemId = editingItem ? editingItem.id : `m-${Date.now()}`;

    const itemData: Mantenimiento = {
      id: itemId,
      userId: user.id,
      fecha,
      tipoMantenimiento,
      ...
      proximoSugeridoKilometraje: isNaN(numProximoKm) ? numKm + 10000 : numProximoKm,
      proximoSugeridaFecha: proximoSugeridaFecha || '2026-12-31',
      taller,
      nroFactura
    };

    if (editingItem) {
      updatedMants = mantenimientos.map(m => m.id === editingItem.id ? itemData : m);
    } else {
      updatedMants = [itemData, ...mantenimientos];
    }

    // 1. Update list of maintenance
    setMantenimientos(updatedMants);
    saveStoredMantenimiento(updatedMants, user.id);

    // 2. Automatically sync user's car mileage with the newly registered maintenance if higher
    if (numKm > user.carKilometers) {
      const users = getStoredUsers();
      const updatedUser = { ...user, carKilometers: numKm };
    ...
    const currentAlerts = getStoredAlertas(user.id);
    ...
    const filteredAlerts = currentAlerts.filter(a => !(a.tipo === 'mantenimiento' && a.mensaje.includes(tipoMantenimiento)));
    ...
    const finalAlerts = [newAlert, ...filteredAlerts];
    setAlertas(finalAlerts);
    saveStoredAlertas(finalAlerts, user.id);
